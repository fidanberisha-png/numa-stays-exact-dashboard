...
(function () {
  ...
  function shell() {
    ...
    el('apply').onclick = function () { nsRun(false); };
    el('refresh').onclick = function () { nsRun(true); };
    el('expandAll').onclick = function () { expandAll(); }; el('vDet').onclick = function () { nsSwitch('details'); }; el('vSum').onclick = function () { nsSwitch('summary'); }; paintView();
  ...
  async function fetchOne(div, year) {
    for (var a = 0; a < 3; a++) {
      try {
        var r = await fetch('/api/accrued/transactions?division=' + div + '&year=' + year + '&code=' + encodeURIComponent(S.code) + (S.fresh ? '&fresh=1' : ''), { credentials: 'same-origin' });
        var j = await r.json();
  ...
  async function fetchSum(div, year, pp, np) { for (var a = 0; a < 2; a++) { try { var r = await fetch('/api/accrued/summary?division=' + div + '&year=' + year + '&code=' + encodeURIComponent(S.code) + ((np && np > 1) ? ('&parts=' + np + '&part=' + pp) : '') + (S.fresh ? '&fresh=1' : ''), { credentials: 'same-origin' }); var j = await r.json(); if (r.ok && j && !j.error) return j; if (r.status === 401) return { error: 'Not connected to Exact Online' }; if (a === 1) return { error: (j && (j.error || j.detail)) || ('HTTP ' + r.status) }; } catch (e) { if (a === 1) return { error: String(e) }; } await sleep(1500); } return { error: 'unknown' }; } async function run() {
    if (S.busy) return;
    S.busy = true;
  ...
  function rowInfo(lbl) {
    if (lbl === NO_CC_LABEL) return infoIcon(NO_CC_INFO);
    if (lbl === NO_GL_LABEL) return infoIcon(NO_GL_INFO);
    return '';
  }
