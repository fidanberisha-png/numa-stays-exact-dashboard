...
(function () {
  ...
  function infoIcon(text) {
    if (!document.getElementById('nsInfoStyle')) {
      var st = document.createElement('style');
      st.id = 'nsInfoStyle';
      st.textContent = '.nsinfo{display:inline-flex;align-items:center;justify-content:center;width:15px;height:15px;border-radius:50%;border:1px solid currentColor;font-size:10px;font-weight:700;line-height:1;margin-left:8px;cursor:help;opacity:.8;position:relative;vertical-align:middle;font-family:inherit}.nsinfo:hover{opacity:1}.nsinfo .nstip{display:none;position:absolute;right:-2px;top:170%;width:300px;padding:9px 11px;border-radius:8px;background:#0f1420;color:#e6edf7;border:1px solid #2a3343;font-size:11px;font-weight:700;line-height:1.5;letter-spacing:0;text-align:left;white-space:normal;z-index:99;box-shadow:0 10px 24px rgba(0,0,0,.5)}.nsinfo:hover .nstip,.nsinfo:focus .nstip{display:block}';
      document.head.appendChild(st);
    }
    return '<span class="nsinfo" tabindex="0" aria-label="' + esc(text) + '">i<span class="nstip">' + esc(text) + '</span></span>';
  }
