...
(function () {
  ...
  function line(label, value, dim) {
    return '<div style="display:flex;justify-content:space-between;gap:16px;padding:4px 0;font-size:0.95em;color:' +
      (dim ? '#8a93a6' : '#e0e0e0') + '"><span>' + label + '</span><span style="font-weight:600;color:' +
      (dim ? '#8a93a6' : '#00bfff') + '">' + fmt(value) + '</span></div>';
  }

  function block(title, idx, highlight) {
    return '<div style="flex:1;min-width:230px;padding:14px 18px;border-radius:8px;background:' +
      (highlight ? '#232c42' : 'transparent') + ';border:1px solid #2c344a">' +
  ...
  function template() {
    return '<div class="chart-card" id="exactResultCard" style="margin-bottom:30px">' +
      '<div style="display:flex;flex-wrap:wrap;align-items:center;gap:12px">' +
        ...
        '<div id="erTotals" style="margin-left:auto;color:#c9d1e1;font-size:0.95em"></div>' +
      '</div>' +
      '<div style="color:#7c869b;font-size:0.85em;margin:6px 0 18px">' + COMPANY +
      ...
      '<div class="filters" style="margin-bottom:20px">' +
        ...
        '<div class="filter-group" id="erRateGroup"><label>Rate to EUR:</label>' +
          ...
          'border:1px solid #2c344a;background:#1a1f2e;color:#e0e0e0"></div>' +
      '</div>' +
      ...
      '<div style="text-align:center;color:#7c869b;margin-top:10px">Result per period</div>' +
    '</div>';
  }
  ...
  function drawChart() {
    ...
    chart = new Chart(el.getContext('2d'), {
        ...
        datasets: [
          {
            ...
            pointRadius: 4, pointBackgroundColor: '#1a1f2e', borderWidth: 2, tension: 0
          },
          {
            label: String(prevYear()), data: series(prevYear()),
            borderColor: '#8a93a6', backgroundColor: '#8a93a6',
            pointRadius: 4, pointBackgroundColor: '#1a1f2e', borderWidth: 2, tension: 0
          }
        ]
      },
      options: {
        ...
        scales: {
          x: { ticks: { color: '#888' }, grid: { color: '#2c344a' } },
          y: { ticks: { color: '#888', callback: function (v) { return short(v); } }, grid: { color: '#2c344a' } }
        }
      }
    });
  }
